...
export const getMapEntities = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    ...
    conditions.push(sql`
      ${buddhashantiAggregateBuilding.building_gps_latitude}::numeric BETWEEN ${south.toString()}::numeric AND ${north.toString()}::numeric AND
      ${buddhashantiAggregateBuilding.building_gps_longitude}::numeric BETWEEN ${west.toString()}::numeric AND ${east.toString()}::numeric
    `);

    // Apply optional filters
    if (wardId) {
      conditions.push(
        eq(buddhashantiAggregateBuilding.ward_number, parseInt(wardId)),
      );
    }

    if (areaCode) {
      conditions.push(
        eq(buddhashantiAggregateBuilding.area_code, parseInt(areaCode)),
      );
    }

    if (enumeratorId) {
      conditions.push(
        eq(buddhashantiAggregateBuilding.enumerator_id, enumeratorId),
      );
    }

    if (mapStatus) {
      conditions.push(eq(buddhashantiAggregateBuilding.map_status, mapStatus));
    }
    ...
    const buildingEntities = await ctx.db
      .select({
        ...
        accuracy: buddhashantiAggregateBuilding.building_gps_accuracy,
      ...
      .orderBy(buddhashantiAggregateBuilding.created_at)
      .limit(limit);
...
export const getMapEntityById = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    ...
    if (type === "building") {
      ...
      const jsonbField =
        type === "household"
          ? buddhashantiAggregateBuilding.households
          : buddhashantiAggregateBuilding.businesses;
...
export const getClusterPoints = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    ...
    const buildingEntities = await ctx.db
      ...
      .where(
        sql`
          ${buddhashantiAggregateBuilding.building_gps_latitude}::numeric BETWEEN ${latMin.toString()}::numeric AND ${latMax.toString()}::numeric AND
          ${buddhashantiAggregateBuilding.building_gps_longitude}::numeric BETWEEN ${lngMin.toString()}::numeric AND ${lngMax.toString()}::numeric
        `,
      )
      .limit(limit);
